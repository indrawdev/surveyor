...
class Survey extends CI_Controller {
	...
	public function survey1($cabang, $noapk) {
		...
		$data['jenis_usaha'] = $this->MSurvey->getReferensi('jenis_usaha');
	...
	public function survey2($cabang, $noapk) {
		...
		$data['kondisi_rumah'] = $this->MSurvey->getRefSurvey('capital_kondisi_rumah');
	...
	public function submitc1() {
		...
		$kartu_keluarga_konsumen = $this->input->post('fs_kartu_keluarga_konsumen');
		...
		$tanggal_terbit_ktp = $this->input->post('fd_tanggal_terbit_ktp');
		...
		$nomor_rt_konsumen = $this->input->post('fs_nomor_rt_konsumen');
		$nomor_rw_konsumen = $this->input->post('fs_nomor_rw_konsumen');
		...
		$tahun_tinggal = $this->input->post('fn_tahun_tinggal');
		$jumlah_tanggungan = $this->input->post('fn_jumlah_tanggungan');
		...
		$kerja_sejak = $this->input->post('fn_kerja_sejak');
		...
		$lama_kerja = $this->input->post('fn_lama_kerja');
		...
		$makelar = $this->input->post('fs_nama_makelar');
		$harga_otr = $this->input->post('fn_harga_otr');
		$jenis_asuransi = $this->input->post('fs_jenis_asuransi');
		...
		$dt = array(
			...
			'fs_kartu_keluarga_konsumen' => trim($kartu_keluarga_konsumen),
			...
			'fd_tanggal_terbit_ktp' => trim($tanggal_terbit_ktp),
			...
			'fs_nomor_rt_konsumen' => trim($nomor_rt_konsumen),
			'fs_nomor_rw_konsumen' => trim($nomor_rw_konsumen),
			...
			'fn_tahun_tinggal' => trim($tahun_tinggal),
			'fn_jumlah_tanggungan' => trim($jumlah_tanggungan),
			'fs_pekerjaan' => trim($pekerjaan),
			'fs_nama_perusahaan' => trim(strtoupper($nama_perusahaan)),
			'fs_jenis_usaha' => trim($jenis_usaha),
			'fn_kerja_sejak' => trim($kerja_sejak),
			'fs_alamat_perusahaan' => trim(strtoupper($alamat_perusahaan)),
			'fs_kota_perusahaan' => trim(strtoupper($kota_perusahaan)),
			'fs_kodepos_perusahaan' => trim($kodepos_perusahaan),
			'fs_jabatan_perusahaan' => trim(strtoupper($jabatan_perusahaan)),
			...
			'fs_no_rangka' => trim(strtoupper($no_rangka)),
			'fs_no_mesin' => trim(strtoupper($no_mesin)),
			'fs_no_faktur' => trim(strtoupper($no_faktur)),
			'fs_komersial' => trim($komersial),
			'fs_nama_bpkb' => trim(strtoupper($nama_bpkb)),
			'fs_nomor_bpkb' => trim(strtoupper($nomor_bpkb)),
			'fs_no_polisi' => trim(strtoupper($no_polisi)),
			'fs_nama_makelar' => trim(strtoupper($makelar)),
			'fn_harga_otr' => trim($harga_otr),
			'fs_jenis_asuransi' => trim($jenis_asuransi),
	...
	public function submitc2() {
		...
		$bukti_kepemilikan_asset = $this->input->post('fs_bukti_kepemilikan_aset');
		...
		$dt = array(
			...
			'fs_bukti_kepemilikan_aset' => trim($bukti_kepemilikan_asset)
		);
	...
	public function submitc3() {
		...
		$tambahan = $this->input->post('fs_pendapatan_tambahan');
		...
		// RINCIAN PENDAPATAN
		$subtotal_omzet = $omzet_perbulan * ($omzet_persen / 100);
		$subtotal_hargasewa = $hargasewa_perbulan * ($hargasewa_persen / 100);
		$total_rincian = $subtotal_omzet + $subtotal_hargasewa;

		// TOTAL 
		$total_pendapatan = $pendapatan_utama + $pendapatan_pasangan + $pendapatan_tambahan + $total_rincian;
		$total_biaya = $biaya_rutin + $angsuran_lain + $biaya_perawatan;

		// SISA PENGHASILAN
		$sisa_penghasilan = $total_pendapatan - $total_biaya;

		$dt = array(
			'fs_usaha_pasangan' => trim($usaha_pasangan),
			'fs_pendapatan_tambahan' => trim($tambahan),
	...
	public function sesuai() {
		...
		if ($update == true) {
			$dt1 = array(
				'fs_user_edit' => trim($user),
				'fd_tanggal_survey' => date('Y-m-d H:i:s'),
				'fd_tanggal_edit' => date('Y-m-d H:i:s')
			);
